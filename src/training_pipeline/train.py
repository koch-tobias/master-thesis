...
def fit_eval_model(X_train: np.array, y_train: np.array, X_val: np.array, y_val: np.array, X_test: np.array, y_test: np.array, weight_factor: float or dict, hp_in_iteration: dict, df: pd.DataFrame, model_results_dict: dict, num_models_trained: int, total_models: int, binary_model: bool, method: str) -> tuple[dict, pd.DataFrame, dict]:
    ...
    train_loss, fbeta_train, val_loss, precision_val, sensitivity_val, fbeta_val, sensitivity_test, precision_test, fbeta_test, df_new = evaluate_model(gbm, X_test, y_test, X_val, y_val, X_train, y_train, evals, hp_in_iteration, num_models_trained, training_time, df.columns, binary_model=binary_model, method=method)    
...
def create_result_df(hp_dict: dict) -> tuple[pd.DataFrame, int]:
    ...
    '''
    fix_columns = ["model_name", "train loss", "train fbeta_score", "validation loss", "validation precision", "validation sensitivity", "validation fbeta_score", "test sensitivity", "test precision", "test fbeta_score", "Training Time (s)", "patience", "Trained iterations"]
...
def grid_search(X_train: np.array, y_train: np.array, X_val: np.array, y_val: np.array, X_test: np.array, y_test: np.array, weight_factor: float or dict, hp_dict: dict, binary_model: bool, method: str) -> tuple[pd.DataFrame, dict, dict]:
    ...
    model_results_dict = {"models_list": [], "evals_list": [], "train_loss": [], "train_fbeta_score": [], "val_loss": [], "val_sensitivity": [], "val_precision": [], "val_fbeta_score": [], "test_sensitivity": [], "test_precision": [], "test_fbeta_score": [], "best_iteration": []}
...
def k_fold_crossvalidation(X: np.array, y: np.array, X_test: np.array, y_test: np.array, weight_factor: float or dict, df: pd.DataFrame, model_results_dict: dict, folder_path: Path, method: str, binary_model: bool) -> pd.DataFrame:
    ...
    fix_columns = ["model_name", "avg train loss","avg train fbeta_score", "avg validation loss",  "avg validation sensitivity", "avg validation precision", "avg validation fbeta_score", "avg test sensitivity", "avg test precision", "avg test fbeta_score"]
    ...
    for i in range(top_x_models):
        cv_results_dict = {"models_list": [], "evals_list": [], "train_loss": [], "train_fbeta_score": [], "val_loss": [], "val_sensitivity": [],  "val_precision": [],  "val_fbeta_score": [], "test_sensitivity": [],  "test_precision": [],  "test_fbeta_score": [], "best_iteration": []}
...
def train_model(folder_path: Path, binary_model: bool, method: str):
    ...
    df = df.sort_values(by=["validation fbeta_score"], ascending=False)
    ...
    df_cv = df_cv.sort_values(by=["avg validation fbeta_score"], ascending=False)
    ...
    store_trained_model(model=best_model, metrics=metrics, best_iteration=best_iteration, val_fbeta=df_cv["avg validation fbeta_score"].iloc[0], hp=best_hp, index_best_model=index_best_model, model_folder_path=model_folder_path, finalmodel=False)
    ...
    y_pred_val, probs, _ = Identifier.model_predict(model=gbm_final, X_test=X_val, method=method, binary_model=binary_model)

    precision_val, sensitivity_val, fbeta_val = calculate_metrics(y_val, y_pred_val, binary_model)

    store_trained_model(model=gbm_final, metrics=metrics, best_iteration=best_iteration, val_fbeta=fbeta_val, hp=best_hp, index_best_model=index_best_model, model_folder_path=model_folder_path, finalmodel=True)
