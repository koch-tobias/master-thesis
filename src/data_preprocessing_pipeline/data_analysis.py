...
def plot_bounding_box(ax, transformed_boundingbox: np.array, designation: str, label_relevant: str) -> int:
    ...
    for edge in edges:
        if label_relevant == config['labels']['binary_label_0']:
            ...
            relevant_count = 0
        elif label_relevant == config['labels']['binary_label_1']:
...
def plot_vehicle(df: pd.DataFrame, add_valid_space: bool, preprocessed_data: bool, mirrored: bool) -> None:
    ...
    for index, row in df.iterrows():
            ...
            relevant = plot_bounding_box(ax=ax, transformed_boundingbox=transformed_boundingbox, designation=row[config['general_params']['car_part_designation']], label_relevant=row[config['labels']['binary_column']])
